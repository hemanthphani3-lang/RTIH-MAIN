    ...
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="flex items-center justify-between bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl">
          <div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight">Platform Admin</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {[
            ...
            <div key={label} className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl shadow-xl border border-white/10">
              <div className="flex items-center gap-3 mb-4">
                <Icon className={`w-5 h-5 text-${color}-400`} />
                <h3 className="text-xs font-bold text-slate-300 tracking-wide uppercase">{label}</h3>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 text-center">
            <p className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-2">Avg Ecosystem Health</p>
            <p className="text-5xl font-extrabold text-[#FFD700] drop-shadow-[0_0_15px_rgba(255,215,0,0.4)]">{avgHealth}</p>
            <p className="text-slate-400 text-sm mt-2">across all startups</p>
          </div>
          <div className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 text-center">
            <p className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-2">Critical Risks</p>
            <p className={`text-5xl font-extrabold ${criticalCount > 0 ? 'text-red-400' : 'text-green-400'}`}>{criticalCount}</p>
            <p className="text-slate-400 text-sm mt-2">startups need intervention</p>
          </div>
          <div className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 text-center">
            <p className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-2">Certificates Issued</p>
            <p className="text-5xl font-extrabold text-green-400">{certificates.length}</p>
            <p className="text-slate-400 text-sm mt-2">total certifications</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-indigo-600/10 to-purple-600/10 border border-indigo-500/30 rounded-3xl p-8 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="relative z-10 shrink-0">
             <div className="bg-black/40 backdrop-blur-md border border-white/10 p-5 rounded-2xl flex items-center gap-4 hover:border-indigo-500/50 transition-colors cursor-pointer group">
               ...
               <div>
                 ...
                 <p className="text-sm text-white italic">"Which domain is growing fastest this month?"</p>
               </div>
             </div>
          </div>
        </div>
        ...
        {riskFlags.length > 0 && (
          <div className="bg-red-500/5 backdrop-blur-xl rounded-3xl border border-red-500/20 overflow-hidden">
            <div className="p-6 border-b border-red-500/20 flex items-center gap-3 bg-red-500/10">
              ...
              <h2 className="text-xl font-bold text-white">Critical Risk Alerts</h2>
        ...
        {certificates.length > 0 && (
          <div className="bg-green-500/5 backdrop-blur-xl rounded-3xl border border-green-500/20 overflow-hidden">
            <div className="p-6 border-b border-green-500/20 flex items-center gap-3 bg-green-500/10">
              ...
              <h2 className="text-xl font-bold text-white">Recent Certifications</h2>
            </div>
            <div className="divide-y divide-white/5">
              {certificates.map((cert) => (
                <div key={cert.id} className="p-5 flex items-center justify-between hover:bg-white/5 transition-all">
                  <div>
                    <p className="font-bold text-white">{cert.organizations?.name}</p>
                    <p className="text-sm text-slate-300 mt-0.5">🏆 {cert.certificate_type}</p>
                  </div>
                  <span className="text-xs text-slate-400">{new Date(cert.issue_date).toLocaleDateString()}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-3xl shadow-xl border border-white/10 overflow-hidden">
          ...
          <div className="divide-y divide-white/10">
            {pendingOrgs.length === 0 ? (
              <p className="p-12 text-center text-slate-400 font-medium">No pending enrollments.</p>
            ) : (
              pendingOrgs.map((req) => (
