...
export default function AdminOpportunitiesAnalyticsPage() {
  ...
  if (loading) return <DashboardLayout><div className="p-8 text-center text-slate-900 dark:text-white">Loading Ecosystem Data...</div></DashboardLayout>;
    ...
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="flex items-center justify-between bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 p-6 rounded-3xl">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Ecosystem Opportunities Overview</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Global visibility into all Hackathons, Challenges, and Grants.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 rounded-3xl">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-slate-500 dark:text-slate-400 font-bold uppercase text-xs tracking-wider">Total Opportunities</h3>
            </div>
            <p className="text-4xl font-extrabold text-slate-900 dark:text-white">{stats.oppsCount}</p>
          </div>
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 rounded-3xl">
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 bg-blue-500/20 rounded-lg"><Target className="w-5 h-5 text-blue-400" /></div>
              <h3 className="text-slate-500 dark:text-slate-400 font-bold uppercase text-xs tracking-wider">Applications Submitted</h3>
            </div>
            <p className="text-4xl font-extrabold text-slate-900 dark:text-white">{stats.appsCount}</p>
          </div>
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 rounded-3xl">
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 bg-green-500/20 rounded-lg"><Activity className="w-5 h-5 text-green-400" /></div>
              <h3 className="text-slate-500 dark:text-slate-400 font-bold uppercase text-xs tracking-wider">Active Participants</h3>
            </div>
            <p className="text-4xl font-extrabold text-slate-900 dark:text-white">{stats.activeCount}</p>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {opportunities.map((opp) => (
                  <tr key={opp.id} className="border-b border-white/5 hover:bg-white dark:bg-white/5 transition-colors">
                    <td className="p-4">
                    ...
                    <td className="p-4 text-sm text-slate-600 dark:text-slate-300">{opp.domains?.name || 'All'}</td>
                    <td className="p-4">
                      <span className={`text-xs font-bold px-2 py-1 rounded border ${opp.status === 'Published' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-slate-500/10 text-slate-500 dark:text-slate-400 border-slate-500/20'}`}>
                        {opp.status}
                      </span>
                    </td>
                    <td className="p-4 text-sm text-slate-600 dark:text-slate-300">{opp.capacity || 'Unlimited'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
