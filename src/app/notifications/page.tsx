  ...
  return (
    <DashboardLayout>
      <div className="p-8 max-w-4xl mx-auto space-y-6 pb-20 animate-in fade-in duration-500">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">Manage your connection requests and alerts</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-3xl overflow-hidden">
            <div className="p-5 border-b border-white/10 flex items-center gap-2">
              ...
              <h2 className="font-bold text-white">Connection Requests</h2>
            ...
            <div className="divide-y divide-white/5">
              {pendingRequests.map(conn => {
                ...
                return (
                  <div key={conn.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-white/5 transition-all">
      ...
      {reviewProfileId && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setReviewProfileId(null)}>
            <div className="bg-white/5 border border-white/20 w-full max-w-sm rounded-3xl p-8 shadow-2xl backdrop-blur-xl animate-in zoom-in-95 duration-300 relative" onClick={e => e.stopPropagation()}>
              <button onClick={() => setReviewProfileId(null)} className="absolute top-4 right-4 p-2 hover:bg-white/10 rounded-xl transition-colors">
                <X className="w-5 h-5 text-slate-400" />
              </button>
              
              <div className="flex flex-col items-center text-center mt-4">
                <div className="w-24 h-24 bg-gradient-to-tr from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 rounded-full flex items-center justify-center text-4xl font-extrabold text-white mb-4 shadow-[0_0_30px_rgba(99,102,241,0.2)]">
                ...
                {(isOrg && reviewContact.description) && (
                  <p className="text-sm text-slate-300 mb-6 italic border-l-2 border-indigo-500/50 pl-3 text-left">
                ...
                <div className="w-full bg-white/5 rounded-2xl p-4 border border-white/5">
                  ...
                  <p className="text-sm text-white font-medium">{reviewContact.email}</p>
                </div>
              </div>
            </div>
          </div>
