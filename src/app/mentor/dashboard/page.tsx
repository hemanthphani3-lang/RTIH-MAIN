...
export default function MentorDashboard() {
  ...
  if (loading) return <DashboardLayout><div className="p-8 text-center text-slate-900 dark:text-white">Loading your portfolio...</div></DashboardLayout>;
  ...
  return (
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        {/* Header */}
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 p-6 rounded-3xl gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Mentor Dashboard</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-white/10 flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-widest">Assigned Startups</p>
              <h3 className="text-4xl font-extrabold text-slate-900 dark:text-white mt-2">{usedCapacity}</h3>
            </div>
          ...
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-white/10 flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-widest">Capacity</p>
              <h3 className="text-4xl font-extrabold text-slate-900 dark:text-white mt-2">{usedCapacity} / {maxCapacity}</h3>
              <div className="w-full bg-white/60 dark:bg-white/10 rounded-full h-1.5 mt-3">
          ...
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-white/10 flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-widest">Slots Available</p>
        ...
        {impact && (
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-white/10 flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2 mb-1"><Target className="w-5 h-5 text-purple-400"/> Mentor Impact Analytics</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Your historical performance and success rate across assigned startups.</p>
            </div>
            <div className="flex gap-8 items-center text-center">
              <div>
                <p className="text-3xl font-extrabold text-slate-900 dark:text-white">{impact.graduated}</p>
                <p className="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 font-bold">Graduated</p>
              </div>
              <div className="w-px h-10 bg-white/60 dark:bg-white/10"></div>
              <div>
                <p className="text-3xl font-extrabold text-green-400">{impact.successRate}%</p>
                <p className="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 font-bold">Success Rate</p>
              </div>
            </div>
          </div>
        )}
        ...
        {riskInsights.length > 0 && (
          <div className="bg-gradient-to-br from-red-500/10 to-orange-500/10 border border-red-500/20 rounded-3xl p-6 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-4">The AI Copilot has flagged the following startups in your portfolio for review:</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {riskInsights.map((risk, i) => (
                  <div key={i} className="bg-slate-200 dark:bg-black/40 backdrop-blur-md border border-slate-200 dark:border-white/10 rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-bold text-slate-600 dark:text-slate-300">{risk.organizations?.name}</span>
                    ...
                    <p className="text-sm text-slate-900 dark:text-white">{risk.insight}</p>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {assignments.length === 0 ? (
              <div className="col-span-2 bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-16 text-center">
                <Building2 className="w-12 h-12 text-slate-500 mx-auto mb-4 opacity-50" />
                <p className="text-slate-500 dark:text-slate-400 font-medium">No startups assigned to you yet. Your Manager will assign them soon.</p>
              </div>
            ) : (
              assignments.map((a: any, idx: number) => {
                const org = a.organizations;
                return (
                  <div key={idx} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-6 flex flex-col gap-5 hover:bg-white/60 dark:bg-white/10 transition-all">
                    <div className="flex items-start justify-between">
                      <div>
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white">{org?.name}</h3>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">{org?.description}</p>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      <span className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-black/20 px-3 py-1.5 rounded-lg border border-white/5">
                    ...
                    <div className="border-t border-slate-200 dark:border-white/10 pt-4 space-y-2">
                      ...
                      <p className="text-sm text-slate-900 dark:text-white font-medium">{org?.user_profiles?.full_name}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">{org?.user_profiles?.email}</p>
                    </div>
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {recommendations.map((rec: any, idx: number) => (
                  <div key={idx} className="bg-slate-200 dark:bg-black/40 backdrop-blur-md border border-slate-200 dark:border-white/10 rounded-2xl p-5 hover:bg-white/60 dark:bg-white/10 hover:border-indigo-500/30 transition-all group">
                    ...
                    <h3 className="font-bold text-slate-900 dark:text-white mb-1">{rec.opportunities.title}</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">{rec.reason} ({rec.score} pt match)</p>
                  </div>
                ))}
              </div>
            </div>
