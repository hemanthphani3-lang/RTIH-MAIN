    ...
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="flex items-center justify-between bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl">
          <div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight">Startup Verification Pipeline</h1>
            <p className="text-slate-400 mt-1 text-sm">Review and approve startups, then assign a Mentor — all in one step.</p>
          </div>
        ...
        <div className="space-y-6">
          {startups.length === 0 ? (
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-16 text-center">
              ...
              <h3 className="text-xl font-bold text-white mb-2">No pending startups</h3>
              <p className="text-slate-400">Your pipeline is clear! Check back later for new applications.</p>
            </div>
          ) : (
            startups.map((org: any) => (
              <div key={org.id} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8 flex flex-col lg:flex-row gap-8 hover:bg-white/10 transition-all">
                ...
                <div className="flex-1 space-y-4">
                  <div>
                    ...
                    <p className="text-slate-400 mt-1 font-medium">{org.description}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-4 border-t border-white/10">
                    <div>
                      ...
                      <p className="text-sm text-slate-300">{org.problem_statement}</p>
                    </div>
                    <div>
                      <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Solution</h4>
                      <p className="text-sm text-slate-300">{org.solution}</p>
                    </div>
                  </div>

                  <div className="flex flex-wrap items-center gap-3 pt-3">
                    <span className="flex items-center gap-1.5 text-xs text-slate-400 bg-black/20 px-3 py-1.5 rounded-lg border border-white/5">
                      <Globe className="w-3.5 h-3.5" /> {org.website || "No website"}
                    </span>
                    <span className="flex items-center gap-1.5 text-xs text-slate-400 bg-black/20 px-3 py-1.5 rounded-lg border border-white/5">
                      <Building2 className="w-3.5 h-3.5" /> {org.stage}
                    </span>
                    <span className="text-xs text-slate-400 bg-black/20 px-3 py-1.5 rounded-lg border border-white/5">
                ...
                {/* Docs & Actions */}
                <div className="w-full lg:w-64 flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-white/10 pt-6 lg:pt-0 lg:pl-8 gap-6">
                  <div>
                      ...
                      <div className="space-y-2">
                        {org.documents.map((doc: any, i: number) => (
                          <a key={i} href={doc.file_url} target="_blank" rel="noreferrer"
                            className="flex items-center gap-2 p-2.5 rounded-xl bg-white/5 border border-white/10 text-sm text-slate-300 hover:text-white hover:bg-white/10 transition-colors">
      ...
      {approvingOrg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          {/* Modal Panel */}
          <div className="relative z-10 w-full max-w-2xl bg-[#0b1121] border border-white/10 rounded-3xl shadow-2xl animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
            ...
            <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/5">
              <div>
                ...
                <p className="text-slate-400 text-sm mt-1">
                  Approving <span className="text-white font-bold">{approvingOrg.name}</span> · Domain:{" "}
                  <span className="text-[#FFD700] font-bold">{approvingOrg.domains?.name}</span>
                </p>
              </div>
              <button onClick={() => setApprovingOrg(null)} className="text-slate-400 hover:text-white transition-colors p-2 rounded-xl hover:bg-white/10">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto space-y-3">
              {mentorsLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-400 font-medium">No mentors available for the <strong>{approvingOrg.domains?.name}</strong> domain in your hub.</p>
                    ...
                    <div key={m.id} onClick={() => setSelectedMentorId(m.id)}
                      ...
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-extrabold text-lg transition-all ${
                          isSelected ? "bg-[#FFD700] text-black" : "bg-white/10 text-slate-300"
                        }`}>
                          {m.user_profiles?.full_name?.[0]}
                        </div>
                        <div>
                          <p className="text-white font-bold">{m.user_profiles?.full_name}</p>
                          <p className="text-slate-400 text-xs">{m.user_profiles?.email}</p>
            ...
            <div className="p-6 border-t border-white/10 flex items-center gap-3">
              <button onClick={() => setApprovingOrg(null)}
                className="flex-1 py-3 rounded-xl font-bold text-sm bg-white/5 text-slate-300 hover:bg-white/10 transition-all border border-white/10">
                Cancel
              </button>
              <button
