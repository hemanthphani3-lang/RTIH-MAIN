    ...
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-slate-300 uppercase tracking-widest">Pending Approvals</p>
          ...
          <div className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-slate-300 uppercase tracking-widest">Healthy Startups</p>
          ...
          <div className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-slate-300 uppercase tracking-widest">At Risk Startups</p>
          ...
          <a href="/manager/opportunities" className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 flex items-center justify-between hover:bg-white/10 hover:border-indigo-500/30 transition-all group">
            <div>
              <p className="text-sm font-bold text-slate-300 uppercase tracking-widest group-hover:text-indigo-400 transition-colors">Opp. Applications</p>
              <div className="flex items-end gap-3 mt-2">
                <h3 className="text-4xl font-extrabold text-indigo-400">{pendingOppApps}</h3>
                <span className="text-sm font-bold text-slate-400 mb-1 flex items-center gap-1 group-hover:text-indigo-400 transition-colors">Review <ChevronRight className="w-3 h-3"/></span>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-br from-indigo-500/10 to-blue-500/10 border border-indigo-500/20 rounded-3xl p-6 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="relative z-10 shrink-0">
             <div className="bg-black/40 backdrop-blur-md border border-white/10 p-4 rounded-2xl flex items-center gap-3">
               <Bot className="w-8 h-8 text-indigo-400" />
               <div>
                 <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Suggested Prompt</p>
                 <p className="text-sm text-white italic">&quot;Show AI startups below 60 health&quot;</p>
               </div>
             </div>
          </div>
        </div>
        ...
        {riskFlags.length > 0 && (
          <div className="bg-red-500/5 backdrop-blur-xl rounded-3xl border border-red-500/20 overflow-hidden">
            <div className="p-6 border-b border-red-500/20 flex items-center gap-3 bg-red-500/10">
              ...
              <h2 className="text-xl font-bold text-white">Active Risk Alerts</h2>
            </div>
            <div className="divide-y divide-white/5">
              {riskFlags.map((flag) => (
                <div key={flag.id} className="p-5 flex items-center justify-between hover:bg-white/5 transition-all">
                  <div>
                    <p className="font-bold text-white">{flag.organizations?.name}</p>
                    <p className="text-sm text-slate-300 mt-0.5">{flag.risk_type}</p>
                  </div>
        ...
        {healthScores.length > 0 && (
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 overflow-hidden">
            <div className="p-6 border-b border-white/10 flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-white">Startup Health Distribution</h2>
            </div>
            <div className="divide-y divide-white/5">
              {healthScores.map((h: any, idx: number) => (
                <div key={idx} className="p-5 flex items-center justify-between hover:bg-white/5 transition-all">
                  <p className="font-bold text-white">{h.organizations?.name}</p>
                  <div className="flex items-center gap-4">
                    <div className="w-32 bg-white/10 rounded-full h-2">
                      <div className="h-2 rounded-full bg-gradient-to-r from-[#FFD700] to-green-400" style={{ width: `${h.current_score}%` }} />
                    </div>
                    <span className="font-bold text-white w-10 text-right">{h.current_score}</span>
                    <span className={`text-xs font-bold px-3 py-1.5 rounded-full border ${
        ...
        <div>
          ...
          {certQueue.length === 0 ? (
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 text-center text-slate-500 text-sm">No certification requests pending your review.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {certQueue.map((req: any) => (
                <div key={req.id} className="bg-white/5 backdrop-blur-xl border border-amber-500/20 rounded-2xl p-5 hover:bg-white/10 transition-all">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="text-white font-bold">{req.organizations?.name}</h3>
                      <p className="text-slate-400 text-xs">{req.organizations?.user_profiles?.full_name}</p>
                    </div>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-xs bg-slate-700 text-slate-300 px-2 py-1 rounded">{req.current_stage}</span>
        ...
        {domainIntel.length > 0 && (
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-6 h-96">
            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><TrendingUp className="w-6 h-6 text-indigo-400"/> Hub Domain Intelligence</h3>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 overflow-hidden">
          ...
          <div className="divide-y divide-white/10">
            {pendingOrgs.length === 0 ? (
              <p className="p-12 text-center text-slate-400 font-medium">No pending applications.</p>
            ) : (
              pendingOrgs.map((req) => (
                <div key={req.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-white/5 transition-all">
                  <div>
                    <h3 className="text-xl font-bold text-white">{req.organizations?.name}</h3>
                    <p className="text-sm text-slate-300 mt-1">Founder: <span className="font-semibold text-white">{req.organizations?.user_profiles?.full_name}</span></p>
      ...
      {reviewingCert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-extrabold text-white">Review Certification</h3>
              <button onClick={() => setReviewingCert(null)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <div className="mb-4 p-3 bg-white/5 rounded-xl">
              <p className="text-white font-bold">{reviewingCert.organizations?.name}</p>
              <p className="text-xs text-slate-400">{reviewingCert.current_stage} → <span className="text-indigo-400 font-bold">{reviewingCert.requested_stage}</span></p>
            </div>
            ...
            <div className="mt-4 mb-4">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Verification Method</p>
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Your Decision</p>
            ...
            <textarea value={certForm.comments} onChange={e => setCertForm({...certForm, comments: e.target.value})}
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white text-sm focus:border-indigo-500/50 focus:outline-none mb-4" />
          ...
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-8 w-full max-w-lg">
            ...
            {(reviewingMilestone.mentor_outcome || reviewingMilestone.mentor_comments) && (
              <div className="mb-4 p-3 bg-green-500/10 border border-green-500/20 rounded-xl">
                ...
                <p className="text-white text-sm font-bold">{reviewingMilestone.mentor_outcome}</p>
                <p className="text-slate-400 text-xs mt-1">{reviewingMilestone.mentor_comments}</p>
              </div>
            )}
