...
export default function ManagerAssignmentsPage() {
  ...
  if (loading) return <DashboardLayout><div className="p-8 text-center text-slate-900 dark:text-white">Loading Assignments...</div></DashboardLayout>;
    ...
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 p-6 rounded-3xl flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Mentorship Assignments</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Match approved startups with the right mentors in your hub.</p>
          </div>
          <div className="flex items-center gap-3">
            <div className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${stage === "select_startup" ? "bg-[#FFD700] text-black border-[#FFD700]" : "bg-white dark:bg-white/5 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/10"}`}>
              1. Select Startup
            </div>
            <ChevronRight className="w-4 h-4 text-slate-500" />
            <div className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${stage === "select_mentor" ? "bg-[#FFD700] text-black border-[#FFD700]" : "bg-white dark:bg-white/5 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/10"}`}>
              2. Pick Mentor
            </div>
          </div>
        </div>
        ...
        {stage === "select_startup" && (
          <div className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-300">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-widest text-sm text-slate-500 dark:text-slate-400">
            ...
            {startups.length === 0 ? (
              <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-16 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">All Assigned!</h3>
                <p className="text-slate-500 dark:text-slate-400">Every approved startup in your hub has a Mentor. Check the Startups pipeline for new applicants.</p>
              </div>
            ) : (
              startups.map((s: any) => (
                <div key={s.id} onClick={() => handleSelectStartup(s)}
                  className="group bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-6 cursor-pointer hover:bg-white/60 dark:bg-white/10 hover:border-[#FFD700]/30 transition-all flex items-center justify-between">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        <span className="text-[10px] bg-white dark:bg-white/5 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-white/10 px-2 py-0.5 rounded-md font-bold uppercase tracking-wider">{s.stage}</span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="flex items-center gap-4">
              <button onClick={() => { setStage("select_startup"); setSelectedStartup(null); }}
                className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors font-medium flex items-center gap-1">
                ← Back
              </button>
              <div className="bg-indigo-500/10 border border-indigo-500/20 px-4 py-2 rounded-xl">
                <p className="text-xs text-slate-500 dark:text-slate-400">Assigning Mentor for</p>
                <p className="text-sm font-bold text-slate-900 dark:text-white">{selectedStartup?.name}</p>
              </div>
            </div>
            ...
            {mentors.length === 0 ? (
              <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-16 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">No Mentors Available</h3>
                <p className="text-slate-500 dark:text-slate-400">There are no Mentors in your hub with capacity in the <strong>{selectedStartup?.domains?.name}</strong> domain.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {mentors.map((m: any) => (
                  <div key={m.id} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-6 flex flex-col gap-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="text-slate-900 dark:text-white font-bold">{m.user_profiles?.full_name}</h3>
                          <p className="text-slate-500 dark:text-slate-400 text-sm">{m.user_profiles?.email}</p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between border-t border-slate-200 dark:border-white/10 pt-4">
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <div className="w-24 bg-white/60 dark:bg-white/10 rounded-full h-1.5">
