    ...
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl">
          <Link href="/manager/opportunities" className="inline-flex items-center gap-2 text-slate-400 hover:text-white mb-4 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back to Opportunities
          </Link>
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-extrabold text-white tracking-tight">{opportunity.title}</h1>
              <p className="text-slate-400 mt-1 text-sm">Review applications for this opportunity.</p>
            </div>
        ...
        <div className="space-y-6">
          {applications.length === 0 ? (
            <div className="bg-white/5 border border-white/10 rounded-3xl p-16 text-center">
              ...
              <p className="text-slate-400">Applications will appear here once organizations submit them.</p>
            </div>
          ) : (
            applications.map((app) => (
              <div key={app.id} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 flex flex-col md:flex-row gap-6 hover:bg-white/10 transition-colors">
                ...
                <div className="flex-1 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-white font-medium">{app.organizations?.stage}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 mb-1 flex items-center gap-1"><User className="w-4 h-4"/> Founder</p>
                      <p className="text-white font-medium">{app.organizations?.user_profiles?.full_name}</p>
                    </div>
                  </div>
                  ...
                  {app.submission_data && (
                    <div className="pt-4 border-t border-white/10">
                      ...
                      <p className="text-sm text-slate-300">{app.submission_data.pitch}</p>
                    </div>
                  )}
                </div>
