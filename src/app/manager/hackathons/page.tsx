...
export default function ManagerHackathons() {
  ...
  if (loading) return <DashboardLayout><div className="p-8 text-center text-slate-900 dark:text-white animate-pulse">Loading your hackathons...</div></DashboardLayout>;
    ...
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-24">
        ...
        <div className="flex items-center justify-between bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 p-6 rounded-3xl">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Hackathon Engine</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Create and manage hackathons, invite judges, track submissions.</p>
          </div>
        ...
        {hackathons.length === 0 ? (
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-3xl p-20 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">No Hackathons Yet</h3>
            <p className="text-slate-500 dark:text-slate-400">Create your first hackathon to start managing teams and submissions.</p>
          </div>
        ) : (
          <div className="space-y-6">
            {hackathons.map((h: any) => {
              ...
              return (
                <div key={h.id} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden">
                  {/* Card Header */}
                  <div className="p-6 flex items-center justify-between cursor-pointer hover:bg-white dark:bg-white/5 transition-all"
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${
                            ...
                            h.status === "Draft" ? "bg-slate-500/20 text-slate-500 dark:text-slate-400 border-slate-500/30" :
                        ...
                        <h2 className="text-xl font-bold text-slate-900 dark:text-white">{h.title}</h2>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{h.location} · {h.mode} · {h.hackathon_judges?.length || 0} judges · {h.hackathon_problem_statements?.length || 0} tracks</p>
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="border-t border-slate-200 dark:border-white/10">
                      {/* Tabs */}
                      <div className="flex border-b border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-black/20">
                        {(["judges", "problems"] as Tab[]).map(t => (
                          <button key={t} onClick={() => setActiveTab(prev => ({ ...prev, [h.id]: t }))}
                            className={`px-6 py-3.5 text-sm font-bold uppercase tracking-widest transition-all ${
                              tab === t ? "text-slate-900 dark:text-white border-b-2 border-[#FFD700]" : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white"
                            }`}>
                      ...
                      {tab === "judges" && (
                        <div className="p-6 space-y-6">
                          {/* Add Judge Form */}
                          <div className="bg-slate-200 dark:bg-black/40 border border-white/5 rounded-2xl p-5">
                            <h4 className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-widest mb-4 flex items-center gap-2"><UserPlus className="w-4 h-4 text-indigo-400" /> Invite New Judge</h4>
                            <div className="flex gap-3">
                              <input type="text" placeholder="Judge Full Name" value={judgeMap[h.id]?.name || ""}
                                ...
                                className="flex-1 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white text-sm focus:outline-none focus:border-indigo-500" />
                              ...
                              <button onClick={() => handleAddJudge(h.id)} disabled={addingJudge === h.id}
                                className="bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white px-6 rounded-xl font-bold text-sm transition-all disabled:opacity-50">
                          ...
                          {h.hackathon_judges?.length === 0 ? (
                            <p className="text-slate-500 dark:text-slate-400 text-sm text-center py-4">No judges added yet. Invite judges above.</p>
                          ) : (
                            <div className="space-y-3">
                              {h.hackathon_judges?.map((j: any) => (
                                <div key={j.id} className="bg-slate-200 dark:bg-black/40 border border-white/5 rounded-2xl p-4 space-y-3">
                                  <div className="flex items-center justify-between">
                                    <div>
                                      <p className="font-bold text-slate-900 dark:text-white">{j.name}</p>
                                      <p className="text-xs text-slate-500 dark:text-slate-400">{j.email}</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      <button onClick={() => setShowLink(p => ({ ...p, [j.id]: !p[j.id] }))}
                                        className="flex items-center gap-1.5 bg-white dark:bg-white/5 hover:bg-white/60 dark:bg-white/10 text-slate-600 dark:text-slate-300 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border border-slate-200 dark:border-white/10">
                      ...
                      {tab === "problems" && (
                        <div className="p-6 space-y-6">
                          {/* Add PS Form */}
                          <div className="bg-slate-200 dark:bg-black/40 border border-white/5 rounded-2xl p-5">
                            <h4 className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-widest mb-4 flex items-center gap-2"><Target className="w-4 h-4 text-green-400" /> Add Challenge Track</h4>
                            <div className="space-y-3">
                              <div className="flex gap-3">
                                <input type="text" placeholder="Problem Statement Title" value={psMap[h.id]?.title || ""}
                                  ...
                                  className="flex-1 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white text-sm focus:outline-none focus:border-green-500" />
                                <input type="text" placeholder="Sponsor (optional)" value={psMap[h.id]?.sponsor || ""}
                                  ...
                                  className="w-48 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white text-sm focus:outline-none focus:border-green-500" />
                              </div>
                              <textarea placeholder="Describe the problem statement in detail..." rows={3} value={psMap[h.id]?.description || ""}
                                ...
                                className="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white text-sm focus:outline-none focus:border-green-500" />
                              <button onClick={() => handleAddProblemStatement(h.id)} disabled={addingPs === h.id}
                                className="bg-green-600 hover:bg-green-500 text-slate-900 dark:text-white px-6 py-2.5 rounded-xl font-bold text-sm transition-all disabled:opacity-50">
                          ...
                          {h.hackathon_problem_statements?.length === 0 ? (
                            <p className="text-slate-500 dark:text-slate-400 text-sm text-center py-4">No problem statements yet.</p>
                          ) : (
                            <div className="space-y-3">
                              {h.hackathon_problem_statements?.map((ps: any, i: number) => (
                                <div key={ps.id} className="bg-slate-200 dark:bg-black/40 border border-white/5 rounded-2xl p-4">
                                  <div className="flex items-start justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <p className="font-bold text-slate-900 dark:text-white">{ps.title}</p>
                                    </div>
                                  ...
                                  <p className="text-sm text-slate-500 dark:text-slate-400 ml-8">{ps.description}</p>
                                </div>
                              ))}
                            </div>
                          )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-2xl bg-[#0a0f1e] border border-slate-200 dark:border-white/10 rounded-3xl shadow-2xl p-8 max-h-[92vh] overflow-y-auto animate-in zoom-in-95 duration-200">

            <div className="flex items-center justify-between mb-8">
              <div>
                <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Step {step} of 2</p>
                <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white">
                  {step === 1 ? "🚀 Create Hackathon" : "⚙️ Hackathon Configuration"}
                </h2>
              </div>
              <button onClick={() => setShowModal(false)} className="bg-white dark:bg-white/5 hover:bg-white/60 dark:bg-white/10 border border-slate-200 dark:border-white/10 p-2 rounded-xl transition-all">
            ...
            <div className="flex gap-2 mb-8">
              <div className={`h-1.5 flex-1 rounded-full transition-all ${step >= 1 ? "bg-[#FFD700]" : "bg-white/60 dark:bg-white/10"}`} />
              <div className={`h-1.5 flex-1 rounded-full transition-all ${step >= 2 ? "bg-[#FFD700]" : "bg-white/60 dark:bg-white/10"}`} />
            </div>

            {step === 1 && (
              <form onSubmit={handleCreateOpportunity} className="space-y-5">
                <div>
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Hackathon Title</label>
                  <input required type="text" value={oppForm.title}
                    onChange={e => setOppForm({ ...oppForm, title: e.target.value })}
                    className="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:border-[#FFD700]"
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Target Domain</label>
                    <select required value={oppForm.domain}
                      onChange={e => setOppForm({ ...oppForm, domain: e.target.value })}
                      className="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white">
                      {ALL_DOMAINS.map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Mode</label>
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Location Hub</label>
                    <input required type="text" value={oppForm.location}
                      onChange={e => setOppForm({ ...oppForm, location: e.target.value })}
                      className="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Max Capacity</label>
                ...
                <div className="flex gap-4 pt-4 border-t border-slate-200 dark:border-white/10">
                  <button type="button" onClick={() => setShowModal(false)}
                    className="flex-1 py-3 rounded-xl font-bold text-slate-900 dark:text-white bg-white dark:bg-white/5 hover:bg-white/60 dark:bg-white/10 transition-colors">Cancel</button>
            ...
            {step === 2 && (
              <form onSubmit={handleConfigureHackathon} className="space-y-5">
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Min Team Size</label>
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Max Team Size</label>
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Submission Deadline</label>
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Rules & Guidelines</label>
                ...
                <div className="flex gap-4 pt-4 border-t border-slate-200 dark:border-white/10">
                  <button type="button" onClick={() => setStep(1)}
                    className="flex-1 py-3 rounded-xl font-bold text-slate-900 dark:text-white bg-white dark:bg-white/5 hover:bg-white/60 dark:bg-white/10 transition-colors">← Back</button>
                  <button type="submit" disabled={creating}
                    className="flex-1 py-3 rounded-xl font-bold text-black bg-[#FFD700] hover:shadow-[0_0_20px_rgba(255,215,0,0.4)] transition-all disabled:opacity-50">
