      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4">
          <div className="flex gap-3">
            <div className="flex-1 flex items-center gap-3 bg-black/40 border border-white/10 rounded-xl px-4 py-3 focus-within:border-indigo-500 transition-all">
            ...
            <button
              ...
              className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 rounded-xl font-bold text-sm transition-all"
            >
              Search
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            {(["all", "Organization", "Mentor", "Manager"] as const).map(t => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-bold border transition-all ${typeFilter === t ? "bg-indigo-600 border-indigo-500 text-white" : "border-white/10 text-slate-400 hover:text-white hover:border-white/30"}`}
              >
                {t === "all" ? "All Types" : t + "s"}
              </button>
            ))}
            <select
              ...
              className="bg-black/40 border border-white/10 text-slate-300 text-xs rounded-full px-3 py-1.5 focus:outline-none"
            >
        ...
        {/* Results */}
        {loading ? (
          <div className="text-center text-slate-400 py-12">Searching the ecosystem...</div>
        ) : results.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {results.map((profile, idx) => {
              const uid = getProfileUserId(profile);
              return (
                <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col gap-4 hover:bg-white/8 hover:border-white/20 transition-all">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {profile._type === "Mentor" && (
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${profile.availability_status === "Available" ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-slate-500/10 text-slate-400 border border-slate-500/20"}`}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {profile._type === "Mentor" && profile.location && (
                      <span className="text-[10px] text-slate-400 flex items-center gap-1"><MapPin className="w-3 h-3" />{profile.location}</span>
                    )}
                    {profile._type === "Manager" && profile.location && (
                      <span className="text-[10px] text-slate-400 flex items-center gap-1"><MapPin className="w-3 h-3" />{profile.location} Hub</span>
                    )}
                  </div>
