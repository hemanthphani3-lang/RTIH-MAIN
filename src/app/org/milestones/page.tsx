    ...
    <DashboardLayout>
      <div className="p-8 flex items-center justify-center min-h-96">
        <div className="text-center">
          ...
          <p className="text-white font-bold animate-pulse">Loading Founder Action Center...</p>
        </div>
      </div>
    </DashboardLayout>
  );
    ...
    return (
      <div className={`bg-[#0f172a]/80 backdrop-blur-xl border rounded-2xl p-6 flex flex-col md:flex-row gap-5 transition-all hover:bg-[#1e293b]/80 group ${
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-4 bg-black/20 rounded-xl p-3 border border-white/5">
            <div className="flex items-center gap-1.5 text-xs font-bold text-slate-300">
              <Zap className={`w-3.5 h-3.5 ${getDifficultyColor(m.difficulty)}`} />
              Difficulty: <span className="text-white font-medium">{m.difficulty || "Medium"}</span>
            </div>
            <div className="flex items-center gap-1.5 text-xs font-bold text-slate-300">
              <Hourglass className="w-3.5 h-3.5 text-blue-400" />
              Est. Time: <span className="text-white font-medium">{m.estimatedTime || "N/A"}</span>
            </div>
            <div className="flex items-center gap-1.5 text-xs font-bold text-slate-300">
          ...
          {isRejected && (
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 mb-4">
              ...
              <p className="text-slate-300 text-sm">{m.submission?.notes || "No comments provided."}</p>
            </div>
          )}
    ...
    <DashboardLayout>
      <div className="p-8 max-w-5xl mx-auto space-y-10 animate-in fade-in duration-500 pb-24">
        ...
        <div>
          ...
          <p className="text-slate-400 mt-2 text-lg">Your operational engine for stage advancement and venture growth.</p>
        </div>
          ...
          <div className="md:col-span-4 bg-[#1e293b] border border-slate-700 rounded-3xl p-8 flex flex-col justify-between shadow-xl">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-bold text-slate-300 uppercase tracking-widest">Next Action</h3>
              </div>
              
              {nextAction ? (
                <>
                  <h4 className="text-xl font-extrabold text-white mb-2 leading-tight">{nextAction.title}</h4>
                  <p className="text-slate-400 text-sm mb-6">{nextAction.description}</p>
                ...
                <>
                  ...
                  <p className="text-slate-400 text-sm mb-6">You have completed all core milestones for this stage.</p>
                ...
                <p className="text-slate-400 text-sm">All set for now!</p>
              )}
            </div>
        ...
        <div>
          <h2 className="text-2xl font-extrabold text-white mb-6 flex items-center gap-3">
            ...
            <span className="text-xs bg-white/10 text-slate-300 px-3 py-1 rounded-full font-bold ml-2">Required</span>
          </h2>
        ...
        {data?.optional?.length > 0 && (
          <div className="pt-8 border-t border-white/5">
            <h2 className="text-xl font-extrabold text-white mb-6 flex items-center gap-3">
              ...
              <span className="text-xs text-slate-400 font-medium bg-slate-900 px-3 py-1 rounded-full border border-slate-800">Boost your Venture Health</span>
            </h2>
      ...
      {submittingMilestone && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-8 w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xl font-extrabold text-white">Complete Milestone</h3>
              <button onClick={() => setSubmittingMilestone(null)} className="text-slate-400 hover:text-white bg-white/5 p-2 rounded-full transition-colors"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 mb-6 mt-4">
              ...
              <p className="text-slate-300 text-xs leading-relaxed">{submittingMilestone.evidenceHint}</p>
            </div>
              ...
              {(["url", "document", "video"] as EvidenceType[]).map((t) => (
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold border transition-all ${
                    evidenceType === t ? "bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-600/20" : "bg-black/40 border-white/5 text-slate-400 hover:bg-white/5 hover:text-white"
                  }`}
                >
            ...
            <div className="space-y-5">
              <div>
                ...
                <input
                  ...
                  placeholder="https://..."
                  className="w-full bg-black/50 border border-white/10 rounded-xl p-3.5 text-white text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none transition-all shadow-inner"
                />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Implementation Notes</label>
                <textarea
                  value={evidenceNote}
            ...
            <div className="flex gap-3 mt-8">
              <button onClick={() => setSubmittingMilestone(null)} className="flex-1 bg-white/5 border border-white/10 text-slate-300 px-4 py-3.5 rounded-xl font-bold text-sm hover:bg-white/10 hover:text-white transition-all">Cancel</button>
              <button
                onClick={handleSubmitEvidence}
                disabled={submitLoading}
                className="flex-[2] bg-indigo-600 text-white px-4 py-3.5 rounded-xl font-extrabold text-sm hover:bg-indigo-500 hover:shadow-lg hover:shadow-indigo-500/30 transition-all disabled:opacity-60 flex items-center justify-center gap-2"
              >
