  ...
  if (!org) return (
    <DashboardLayout>
      <div className="p-8 max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[70vh] text-center">
        ...
        <h2 className="text-3xl font-extrabold text-white mb-3">Welcome to RTIH!</h2>
        <p className="text-slate-400 mb-8">You haven't registered your startup yet. Get started by completing the onboarding wizard.</p>
  ...
  return (
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        {/* Header */}
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight flex items-center gap-3">
              <Building2 className="w-8 h-8 text-[#FFD700]" /> {org.name}
            </h1>
            <p className="text-slate-400 mt-1">{org.description}</p>
          </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl">
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">Verification Journey</h2>
            ...
            {[
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center relative">
                  {i > 0 && (
                    <div className={`absolute left-0 top-5 -translate-y-1/2 w-full h-0.5 ${isCompleted ? "bg-[#FFD700]" : "bg-white/10"}`} style={{ left: "-50%", width: "100%" }} />
                  )}
                  <div className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm border-2 transition-all ${isCompleted ? "bg-[#FFD700] border-[#FFD700] text-black shadow-[0_0_15px_rgba(255,215,0,0.4)]" : "bg-slate-800 border-white/10 text-slate-500"}`}>
                    {isCompleted ? <CheckCircle2 className="w-5 h-5" /> : i + 1}
                  </div>
                  <p className="text-xs font-bold text-white mt-3">{step.label}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Startup Details */}
          <div className="lg:col-span-2 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 space-y-5">
            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Startup Profile</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <p className="text-white text-sm">{new Date(org.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 flex flex-col">
            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4">Your Mentor</h2>
            {mentor ? (
              <div className="flex flex-col gap-4 flex-grow">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-bold">{mentor?.user_profiles?.full_name}</p>
                    <p className="text-slate-400 text-xs">{mentor?.user_profiles?.email}</p>
                  </div>
                </div>
        ...
        {skillGaps.length > 0 && (
          <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-3xl p-6 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-400 text-sm mb-4">Based on your {org.stage} stage in the {org.domains?.name} domain, your AI Copilot recommends developing the following skills:</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {skillGaps.map((gap, i) => (
                  <div key={i} className="bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl p-4 flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-xs bg-white/10 text-slate-300 px-2 py-1 rounded font-bold">{gap.category}</span>
                      ...
                      <h3 className="font-bold text-white mt-1">{gap.skill}</h3>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {org.documents && org.documents.length > 0 && (
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6">
            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4">Submitted Documents</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {org.documents.map((doc: any, i: number) => (
                <a key={i} href={doc.file_url} target="_blank" rel="noreferrer"
                  className="flex items-center gap-3 p-4 bg-black/20 border border-white/10 rounded-xl hover:bg-white/5 transition-colors group">
                  <div className="w-10 h-10 bg-indigo-500/20 border border-indigo-500/20 rounded-xl flex items-center justify-center">
                    <FileText className="w-5 h-5 text-indigo-400" />
                  </div>
                  <div>
                    <p className="text-white text-sm font-bold group-hover:text-indigo-400 transition-colors">{doc.document_type}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {recommendations.slice(0, 3).map((rec: any, idx: number) => (
                  <div key={idx} onClick={() => router.push(`/org/opportunities/${rec.opportunities.id}`)}
                    className="bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl p-5 hover:bg-white/10 hover:border-indigo-500/30 transition-all cursor-pointer group">
                    ...
                    <h3 className="font-bold text-white group-hover:text-indigo-400 transition-colors mb-1">{rec.opportunities.title}</h3>
