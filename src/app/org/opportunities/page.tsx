  ...
  return (
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-10 animate-in fade-in duration-500 pb-20">
        ...
        {/* Header */}
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/20 rounded-full blur-3xl pointer-events-none" />
          <div className="relative z-10">
            <h1 className="text-4xl font-extrabold text-white tracking-tight flex items-center gap-3">
              <Rocket className="w-8 h-8 text-indigo-400" /> Opportunities Hub
            </h1>
            <p className="text-slate-400 mt-2 max-w-xl">Discover hackathons, grants, funding calls, and ecosystem programs tailored for your startup.</p>
          </div>
          <div className="relative z-10 w-full md:w-auto">
            <Link href="/org/applications" className="bg-white/10 hover:bg-white/20 border border-white/10 text-white px-6 py-3 rounded-xl font-bold transition-colors inline-block w-full md:w-auto text-center">
              My Applications
            </Link>
          </div>
        </div>
              ...
              {recommended.map(({ score, reason, opportunities: opp }) => (
                <div key={opp.id} className="bg-gradient-to-br from-[#FFD700]/10 to-[#FFD700]/5 border border-[#FFD700]/30 rounded-3xl p-6 flex flex-col relative group">
                  ...
                  <div className="mb-4 pt-4">
                    ...
                    <h3 className="text-lg font-bold text-white mt-1 leading-tight">{opp.title}</h3>
                  </div>
                  <p className="text-xs text-slate-400 mb-6 flex-grow">{reason}</p>
        ...
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h2 className="text-xl font-bold text-white">Ecosystem Opportunities</h2>
            <div className="relative w-full md:w-72">
              ...
              <input 
                ...
                className="w-full bg-black/40 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white focus:border-indigo-500 transition-colors"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredOpps.map((opp) => (
              <Link key={opp.id} href={`/org/opportunities/${opp.id}`} className="group bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl p-5 flex flex-col md:flex-row gap-6 transition-all items-center">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] text-indigo-400 font-bold uppercase tracking-wider">{opp.type}</span>
                    <span className="text-slate-600 text-[10px]">•</span>
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{opp.domains?.name || 'All Domains'}</span>
                  </div>
                  <h3 className="text-lg font-bold text-white group-hover:text-indigo-400 transition-colors">{opp.title}</h3>
                  <p className="text-sm text-slate-400 mt-1 line-clamp-1">{opp.description}</p>
                </div>
                ...
                <div className="flex items-center gap-6 w-full md:w-auto">
                  ...
                  <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-indigo-500/20 group-hover:text-indigo-400 transition-colors ml-auto md:ml-0">
