...
export default function OrgOpportunityDetailPage() {
  ...
  if (loading) return <DashboardLayout><div className="p-8 text-center text-slate-900 dark:text-white">Loading Opportunity Details...</div></DashboardLayout>;
  ...
  return (
    <DashboardLayout>
      <div className="p-8 max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <Link href="/org/opportunities" className="inline-flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
          <ArrowLeft className="w-4 h-4" /> Back to Hub
        </Link>

        {/* Header */}
        <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 p-8 md:p-12 rounded-[2.5rem] relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-start">
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-6 leading-tight">{opportunity.title}</h1>
            
            <div className="flex flex-wrap gap-6 text-sm text-slate-600 dark:text-slate-300 bg-slate-200 dark:bg-black/40 p-4 rounded-2xl border border-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-8">
            <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-3xl p-8">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">About the Opportunity</h3>
              <p className="text-slate-600 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{opportunity.description}</p>
            </div>

            <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-3xl p-8">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Eligibility Requirements</h3>
              <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
                <li className="flex gap-3">
                  <CheckCircle2 className="w-5 h-5 text-indigo-400 shrink-0" /> 
                  <div>
                    <span className="font-bold text-slate-900 dark:text-white block mb-0.5">Startup Stages</span>
                ...
                <li className="flex gap-3">
                  <CheckCircle2 className="w-5 h-5 text-indigo-400 shrink-0" /> 
                  <div>
                    <span className="font-bold text-slate-900 dark:text-white block mb-0.5">Domains</span>
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-b from-indigo-500/10 to-transparent border border-indigo-500/20 rounded-3xl p-6 text-center flex flex-col">
              <h3 className="font-bold text-slate-900 dark:text-white mb-6">Application Status</h3>
              ...
              {application ? (
                <div className="flex flex-col items-center justify-center flex-grow space-y-4">
                  ...
                  <div>
                    <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-widest font-bold mb-1">Current Status</p>
                  ...
                  {application.status === 'Accepted' && (
                    <Link href={`/org/workspace/${opportunity.id}`} className="mt-4 w-full py-3 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-slate-900 dark:text-white font-bold transition-colors">
                      Enter Workspace
                    </Link>
                  )}
                </div>
                ...
                <div className="flex flex-col items-center justify-center flex-grow space-y-4">
                  <p className="text-sm text-slate-500 dark:text-slate-400">You have not applied for this opportunity yet.</p>
            ...
            <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-3xl p-6">
               <h3 className="font-bold text-slate-900 dark:text-white mb-4 text-sm uppercase tracking-widest">Timeline</h3>
               <div className="space-y-4 text-sm">
                 <div className="flex justify-between">
                   <span className="text-slate-500 dark:text-slate-400">Applications Open</span>
                   <span className="text-slate-900 dark:text-white font-medium">{opportunity.registration_start_date ? new Date(opportunity.registration_start_date).toLocaleDateString() : 'Now'}</span>
                 </div>
                 <div className="flex justify-between">
                   <span className="text-slate-500 dark:text-slate-400">Deadline</span>
                   <span className="text-slate-900 dark:text-white font-medium">{opportunity.registration_end_date ? new Date(opportunity.registration_end_date).toLocaleDateString() : 'TBD'}</span>
                 </div>
                 <div className="flex justify-between">
