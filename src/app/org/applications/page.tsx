      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="flex items-center justify-between bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl">
          <div>
            ...
            <p className="text-slate-400 mt-1 text-sm">Track your progress and status for all opportunity applications.</p>
          </div>
        ...
        <div className="space-y-6">
          {applications.length === 0 ? (
            <div className="bg-white/5 border border-white/10 rounded-3xl p-16 text-center">
              ...
              <p className="text-slate-400 mb-6">You haven't applied to any opportunities. Head to the Hub to get started.</p>
            ...
            applications.map((app) => (
              <div key={app.id} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 hover:bg-white/10 transition-colors group">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Applied {new Date(app.created_at).toLocaleDateString()}</span>
                  </div>
                ...
                <div className="flex items-center gap-6 w-full md:w-auto">
                  ...
                  {app.status === 'Accepted' ? (
                    <Link href={`/org/workspace/${app.opportunities?.id}`} className="flex items-center gap-2 text-sm font-bold bg-indigo-500 hover:bg-indigo-600 text-white px-5 py-2.5 rounded-xl transition-colors">
                      Workspace <ArrowRight className="w-4 h-4" />
                    </Link>
                  ) : (
                    <Link href={`/org/opportunities/${app.opportunities?.id}`} className="flex items-center gap-2 text-sm font-bold bg-white/5 hover:bg-white/10 border border-white/10 text-white px-5 py-2.5 rounded-xl transition-colors">
                      View Details
                    </Link>
                  )}
                </div>
