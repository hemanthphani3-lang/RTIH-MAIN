...
export default function OrgRoadmapPage() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-white text-lg font-bold animate-pulse">Loading your Venture Roadmap...</div>
    </div>
  );
      ...
      <div className="p-8 max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="flex items-center justify-between bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl">
          <div>
            ...
            <p className="text-slate-400 mt-1 text-sm">Your verified startup journey through the RTIH ecosystem</p>
          </div>
          <div className="bg-[#FFD700]/10 border border-[#FFD700]/20 px-4 py-2 rounded-xl">
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-0.5">Verified Stage</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-white font-bold text-sm">{verifiedStage}</span>
            </div>
          </div>
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4">
            ...
            {org?.claimed_stage && org.claimed_stage !== verifiedStage ? (
              ...
              <span className="text-slate-400 text-sm">Same as verified</span>
            )}
          </div>
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4">
            <p className="text-xs text-slate-500 uppercase tracking-widest font-bold mb-2">Health Score</p>
            <div>
              <span className="text-white font-extrabold text-xl">{org?.health_score || 0}</span>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 relative overflow-hidden">
          ...
          <h2 className="text-xl font-extrabold text-white mb-8 flex items-center gap-2">Stage Progression</h2>
              ...
              return (
                <div key={stage} className="flex gap-5 relative">
                  {i !== VENTURE_STAGES.length - 1 && (
                    <div className={`absolute left-6 top-14 bottom-[-24px] w-0.5 rounded-full ${isCompleted ? "bg-[#FFD700]" : "bg-white/10"}`} />
                  )}
                  ...
                  <div className={`pt-1.5 flex-1 ${isFuture ? "opacity-40" : ""}`}>
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className={`text-lg font-extrabold ${isCurrent ? "text-indigo-300" : isCompleted ? "text-[#FFD700]" : "text-slate-400"}`}>
                        {badge?.icon} {stage}
                      </h3>
        ...
        {activeRequest && (
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6">
            ...
            <div className="flex items-center gap-2 mb-6">
              <span className="text-slate-400 text-sm">Applying for:</span>
              <span className="text-white font-bold">{activeRequest.requested_stage}</span>
            </div>
            {/* 2-Step Tracker */}
                  ...
                  <div key={step} className="flex items-center">
                    <div className="flex flex-col items-center">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border-2 ${
                        isStepDone ? "bg-green-500 border-green-500 text-white" :
                    ...
                    {i < 1 && <div className={`h-0.5 w-24 mx-2 mb-4 ${isStepDone ? "bg-green-500" : "bg-white/10"}`} />}
                  </div>
                );
              })}
            </div>
            {activeRequest.status === "Revision Requested" && (
              <div className="mt-4 p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
                ...
                <p className="text-slate-300 text-sm mt-1">{activeRequest.manager_comments}</p>
              </div>
            )}
        ...
        {!activeRequest && nextStage && (
          <div className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-3xl p-8 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-2xl font-extrabold text-white mb-2">Ready to Advance?</h2>
              <p className="text-slate-400 mb-6">Complete all core milestones, then apply for <span className="text-indigo-400 font-bold">{nextStage}</span> certification.</p>
              <button
                onClick={() => setShowCertModal(true)}
                className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-8 py-3 rounded-xl font-bold text-sm hover:shadow-[0_0_30px_rgba(99,102,241,0.4)] transition-all flex items-center gap-2 mx-auto"
              >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-slate-900 border border-white/10 rounded-3xl p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-extrabold text-white">Apply for {nextStage} Certification</h3>
                <button onClick={() => setShowCertModal(false)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <p className="text-slate-400 text-sm mb-6">Submit evidence for the following requirements:</p>
              <div className="space-y-4">
                {nextStageConfig.advancementEvidence.map((ev) => (
                  <div key={ev}>
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">{ev}</label>
                    <input
                      value={certEvidence[ev] || ""}
                      onChange={(e) => setCertEvidence({ ...certEvidence, [ev]: e.target.value })}
                      placeholder="URL or brief description..."
                      className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white text-sm focus:border-indigo-500/50 focus:outline-none transition-all"
                    />
                  </div>
                ))}
                <div>
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Supporting Notes</label>
                  <textarea
                    value={certNotes}
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => setShowCertModal(false)} className="flex-1 bg-white/5 border border-white/10 text-slate-300 px-4 py-3 rounded-xl font-bold text-sm hover:bg-white/10 transition-all">Cancel</button>
                <button
                  onClick={handleCertSubmit}
                  disabled={certLoading}
                  className="flex-1 bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-4 py-3 rounded-xl font-bold text-sm hover:shadow-[0_0_20px_rgba(99,102,241,0.4)] transition-all disabled:opacity-60"
                >
